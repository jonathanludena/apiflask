...
def select_task_by_id(_id):
    ...
    try:
        conn.row_factory = sqlite3.Row  # para recibir los datos en forma de diccionario
        cur = conn.cursor()
        cur.execute(sql)
        # recibimos un solo valor y lo convertimos a diccionario
...
def completed_task(_id, complete):
    conn = create_connection()

    sql = f""" UPDATE tasks SET completed = {complete}
            WHERE id = {_id}
    """
